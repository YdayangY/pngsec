...
impl Png {
    ...
    pub fn from_file<P: AsRef<Path>>(path: P) -> Result<Self> {
        let mut png_data = File::open(path)?;
        let mut header = [0;8];
        png_data.read_exact(&mut header)?;
        if header != Self::STANDARD_HEADER {
            return Err(Box::new(ParseError { details: "错误的文件类型".to_string()}));
        }
        
        let mut chunks: Vec<Chunk> = Vec::new();
        loop {
            let mut data_length = [0;4];
            if png_data.read_exact(&mut data_length).is_err() {
                break;
            }
            ...
            let mut message_bytes = vec![0;data_length as usize];
            png_data.read_exact(&mut message_bytes);

            let mut crc = [0;4];
            png_data.read_exact(&mut crc);
            let crc = u32::from_be_bytes(crc);

            let crc_calculate = Chunk::get_crc(&chunk_type, &message_bytes);
            if crc != crc_calculate {
                return Err(Box::new(ParseError { details: "错误的crc".to_string()}));
            }
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        if let Some(pos) = &self.chunks.iter().position(|x| str::from_utf8(x.chunk_type().bytes().as_ref()).unwrap() == chunk_type) {
    ...
    pub fn chunk_by_type(&self, chunk_type: &str) -> Option<&Chunk> {
        if let Some(pos) = &self.chunks.iter().position(|x| str::from_utf8(&x.chunk_type().bytes()).unwrap() == chunk_type) {
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        let mut vecs = Self::STANDARD_HEADER.clone().to_vec();
        vecs.extend(self.chunks.iter().flat_map(|x| x.as_bytes()));
        vecs
    }

    pub fn create_png(&self, filename: &str) -> Result<String> {
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Self> {
        let headers = &bytes[0..8];
        let mut len = headers.len();
        if headers != Self::STANDARD_HEADER {
            return Err(Box::new(ParseError {details: "PNG文件头异常".to_string()}));
        }
        let mut chunks: Vec<Chunk> = Vec::new();
        while len < bytes.len() {
            let data_len = read_u32(&bytes[len..len + 4]) as u32;
            len += 4;
            let chunk_type = &bytes[len..len + 4];
            len += 4;
            let chunk_type = ChunkType {
                contents: (chunk_type.try_into().unwrap())
            };
            let chunk_data = &bytes[len..len + (data_len as usize)];
            len += data_len as usize;
            let crc_png: u32 = read_u32(&bytes[len..len+4]);
            let crc = Chunk::get_crc(&chunk_type, &chunk_data.to_vec());
            if crc != crc_png {
                return Err(Box::new(ParseError {details: "crc校验不合法".to_string()}));
            }
            let chunk = Chunk {
                data_length: data_len,
                chunk_type: chunk_type,
                message_bytes: chunk_data.to_vec(),
                crc: crc,
            };
            len += 4;
            chunks.push(chunk);
...
impl fmt::Display for Png {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        for ele in &self.chunks {
            writeln!(f, "  Length: {}", ele.length())?;
            writeln!(f, "  Type: {}", ele.chunk_type())?;
            writeln!(f, "  Data: {} bytes", ele.data().len())?;
            writeln!(f, "  Crc: {},", ele.crc())?;
        }
        writeln!(f, "}}",)?;
        Ok(())
    }
...
mod tests {
    ...
    #[test]
    fn create_png_from_bytes() {
        let png = Png::try_from(&PNG_FILE[..]).unwrap();
        let actual = png.create_png("dict.png").unwrap();
        let expected  = "dict.png".to_string();
        assert_eq!(actual, expected);
    }
